...
def parse_json_to_mlmd(
    mlmd_json, path_to_store: str, cmd: str, exec_id: Union[str, int]
) -> Union[str, None]:
    try:
        ...
        cmf_class = cmf.Cmf(
            filepath=path_to_store,
            pipeline_name=pipeline_name,  # intializing cmf
            graph=graph,
            is_server=True,
        )
        ...
        for stage in data["Pipeline"][0]["stages"]:  # Iterates over all the stages
            if exec_id is None:  # if exec_id is None we pass all the executions.
                ...
                exec_id is not None
            ):  # elif exec_id is not None, we pass executions for that specific id.
                list_executions = [
                    execution
            ...
            for execution in list_executions:  # Iterates over all the executions
                ...
                try:
                    ...
                    _ = cmf_class.update_execution(
                        execution["id"], execution["custom_properties"]
                    )
                except Exception as e:
                ...
                for event in execution["events"]:  # Iterates over all the events
                    ...
                    try:
                        if artifact_type == "Dataset":
                            ...
                            cmf_class.log_execution_metrics_from_client(
                                event["artifact"]["name"], custom_props
                            )
                        elif artifact_type == "Dataslice":
                            dataslice = cmf_class.create_dataslice(
                                event["artifact"]["name"]
                            )
                            dataslice.commit_existing(uri, custom_props)
                        elif artifact_type == "Step_Metrics":
                            cmf_class.commit_existing_metrics(
                                event["artifact"]["name"], uri, custom_props
                            )
                        else:
                            pass
                    except AlreadyExistsError as e:
