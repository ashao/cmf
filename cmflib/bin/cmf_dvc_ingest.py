...
def main(cmf_filename):
    ...
    cmd_exe = {}
    cmf_query = cmfquery.CmfQuery(cmf_filename)
    pipelines = cmf_query.get_pipeline_names()
    for pipeline in pipelines:
        pipeline_name = pipeline
        stages = cmf_query.get_pipeline_stages(pipeline)
        for stage in stages:
            exe_df = cmf_query.get_all_executions_in_stage(stage)
    ...
    pipeline_name = (
        "Pipeline" + "-" + str(uuid_) if not pipeline_name else pipeline_name
    )
        ...
        for kk, vv in v.items():
            for kkk, vvv in vv.items():
                if kkk == "cmd":
                    ...
                    ingest_metadata(
                        lineage, vv, metawriter, tracked, str(" ".join(vvv))
                    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="CMF DVC Ingester",
        description=(
            "Ingest the metadata in dvc.lock file into CMF.  If cmf mlmd file exist with metadata"
            + "from metrics logging or logging for other metadata not captured in dvc.lock file, pass"
            + "that mlmd file as the input file.  This code queries the file for existing pipelines,"
            + "stages and executions and stores as a dictionary. If the execution cmd in the stored"
            + "dict, matches the execution command in the dvc.lock file, that execution is updated with"
            + "additional metadata.  If there is no prior execution captured, a new execution is"
            + "created"
        ),
    )
    parser.add_argument(
