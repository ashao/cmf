...
def generate_cached_url(url, cache):
    ...
    cached_url = (
        parsed_cache_url.scheme + "://" + parsed_cache_url.netloc + parsed_url.path
    )
    return cached_url
...
def download_and_verify_file(
    host, headers, remote_file_path, local_path, artifact_hash, timeout
):
    print(f"Fetching artifact={local_path}, surl={host} to {remote_file_path}")
    data = None
    try:
        response = requests.get(
            host, headers=headers, timeout=timeout, verify=True
        )  # This should be made True. otherwise this will produce Insecure SSL Warning
    ...
    if data is not None:
        try:
            with open(remote_file_path, "wb") as file:
                file.write(data)
            if (
                os.path.exists(remote_file_path)
                and os.path.getsize(remote_file_path) > 0
            ):
...
class OSDFremoteArtifacts:
    def download_artifacts(
        ...
        headers = {
            dvc_config_op["remote.osdf.custom_auth_header"]: dvc_config_op[
                "remote.osdf.password"
            ]
        }
        ...
        if cache == "":
            # Fetch from Origin
            success, result = download_and_verify_file(
                host,
                headers,
                remote_file_path,
                local_file_path,
            ...
            success, cached_result = download_and_verify_file(
                cached_s_url,
                headers,
                remote_file_path,
                local_path,
            ...
            if success:
                ...
                # Fetch from Origin
                success, origin_result = download_and_verify_file(
                    host,
                    headers,
                    remote_file_path,
                    local_path,
                ...
                if success:
                    ...
                    return (
                        success,
                        f"Failed to download and verify file: {origin_result}",
                    )
