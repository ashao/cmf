    ...
    def run(self):
        ...
        if df.empty:
            ...
            if "Python_Env" in df.columns:
                df = df.drop(
                    ["Python_Env"], axis=1
                )  # Type of df is series of integers.

            # Process execution ID if provided
            if (
                not self.args.execution_id
            ):  # If self.args.execution_id is None or an empty list ([]).
                pass
            elif (
                len(self.args.execution_id) > 1
            ):  # If the user provided more than one execution_id.
                raise DuplicateArgumentNotAllowed("execution_id", "-e")
            elif not self.args.execution_id[
                0
            ]:  # self.args.execution_id[0] is an empty string ("").
                raise MissingArgument("execution id")
            else:
                if self.args.execution_id[0].isdigit():
                    if int(self.args.execution_id[0]) in list(
                        df["id"]
                    ):  # Converting series to list.
                        df = df.query(
                            f"id == {int(self.args.execution_id[0])}"
                        )  # Used dataframe based on execution id
                        ...
                        updated_columns += [
                            col for col in df.columns if col not in updated_columns
                        ]
                        ...
                        df = df.T.reset_index()
                        df.columns.values[0] = (
                            "id"  # Rename the first column back to 'id'.
                        )
