...
class CmdArtifactsList(CmdBase):
    def convert_to_datetime(self, df: pd.DataFrame, col_name: str) -> pd.DataFrame:
        ...
        df = df.copy()
        df[col_name] = pd.to_datetime(df[col_name], unit="ms", utc=True).dt.strftime(
            "%a %d %b %Y %H:%M:%S GMT"
        )
    ...
    def display_table(self, df: pd.DataFrame) -> None:
        ...
        for col in df.select_dtypes(include=["object"]).columns:
            df[col] = df[col].apply(
                lambda x: textwrap.fill(x, width=14) if isinstance(x, str) else x
            )
        ...
        while True:
            ...
            user_input = (
                input("Press Enter to see more or 'q' to quit: ").strip().lower()
            )
            if user_input == "q":
                break
    ...
    def search_artifact(self, df: pd.DataFrame) -> Union[int, List[int]]:
        ...
        for index, row in df.iterrows():
            ...
            ):  # Match only the file name: train.tsv
                matched_ids.append(row["id"])
            elif (
                artifact_name == row["name"]
            ):  # Match the full path with hash: artifacts/parsed/train.tsv:12345
                matched_ids.append(row["id"])
            elif (
                artifact_name == row["name"].split("/")[-1]
            ):  # Match only the file name with hash: train.tsv:12345
                matched_ids.append(row["id"])
    ...
    def run(self):
        ...
        if (
            not self.args.file_name
        ):  # If self.args.file_name is None or an empty list ([]).
            mlmd_file_name = "./mlmd"  # Default path for mlmd file name.
        elif (
            len(self.args.file_name) > 1
        ):  # If the user provided more than one file name.
            raise DuplicateArgumentNotAllowed("file_name", "-f")
        elif not self.args.file_name[
            0
        ]:  # self.args.file_name[0] is an empty string ("").
        ...
        elif not self.args.pipeline_name[
            0
        ]:  # self.args.pipeline_name[0] is an empty string ("").
        ...
        if df.empty:
            ...
            if (
                not self.args.artifact_name
            ):  # If self.args.artifact_name is None or an empty list ([]).
                pass
            elif (
                len(self.args.artifact_name) > 1
            ):  # If the user provided more than one artifact_name.
                raise DuplicateArgumentNotAllowed("artifact_name", "-a")
            elif not self.args.artifact_name[
                0
            ]:  # self.args.artifact_name[0] is an empty string ("").
                ...
                if artifact_ids != -1:
                    ...
                    for artifact_id in artifact_ids:
                        ...
                        filtered_data = self.convert_to_datetime(
                            filtered_data, "create_time_since_epoch"
                        )
                        filtered_data = self.convert_to_datetime(
                            filtered_data, "last_update_time_since_epoch"
                        )
                        ...
                        user_input = (
                            input(
                                "Press Enter to see more records if exists or 'q' to quit: "
                            )
                            .strip()
                            .lower()
                        )
