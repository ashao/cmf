...
class GraphDriver:
    ...
    def create_stage_node(
        self, name: str, parent_context: mlpb.Context, stage_id: int, props=None
    ):
        if props is None:
            props = {}
    ...
    def create_dataslice_node(
        self, name: str, path: str, uri: str, parent_name: str, custom_properties=None
    ):
        if custom_properties is None:
            custom_properties = {}
        ...
        with self.driver.session() as session:
            node = session.write_transaction(self._run_transaction, dataslice_syntax)
            node_id = node[0]["node_id"]
    ...
    def create_links(self, source_path: str, target_path: str, relation: str):
        ...
        with self.driver.session() as session:
            pc_syntax = self._create_parent_child_syntax(
                "Dataset", "Dataslice", source_node_id, target_node_id, relation
            )
            _ = session.write_transaction(self._run_transaction, pc_syntax)
    ...
    def create_artifact_relationships(
        self, parent_artifacts, child_artifact, relation_properties
    ):
        #      f = lambda d: d['Event']
    ...
    def create_execution_links(
        self, parent_artifact_uri, parent_artifact_name, parent_artifact_type
    ):

        parent_execution_query = (
        ...
        already_linked_execution_query = (
            "MATCH (f)-[r:linked]->(e2:Execution) "
            "WHERE r.uri = '{}' RETURN ID(f)as id, f.uri as uri".format(
                parent_artifact_uri
            )
        )
    ...
    def _get_node(self, node_label: str, node_name: str) -> int:
        ...
        search_syntax = "MATCH (n:{}) where '{}' in n.Name  \
                              return ID(n) as node_id".format(
            node_label, node_name
        )
        print(search_syntax)
        node_id = None
        with self.driver.session() as session:
            nodes = session.read_transaction(self._run_transaction, search_syntax)
    ...
    @staticmethod
    def _create_dataslice_syntax(name: str, path: str, uri: str, custom_properties):
        custom_properties["Name"] = name
        custom_properties["Path"] = path
    ...
    @staticmethod
    def _create_model_syntax(
        name: str, uri: str, pipeline_id: int, pipeline_name: str, custom_properties
    ):
        custom_properties["Name"] = name
        custom_properties["pipeline_id"] = str(pipeline_id)
        custom_properties["pipeline_name"] = pipeline_name
    ...
    @staticmethod
    def _create_stage_syntax(
        name: str, props: t.Dict, uri: int, pipeline_id: int, pipeline_name: str
    ) -> str:
        props["Name"] = name
        props["uri"] = str(uri)
    ...
    @staticmethod
    def _create_parent_child_syntax(
        parent_label: str,
        child_label: str,
        parent_id: int,
        child_id: int,
        ...
        parent_child_syntax = "MATCH (a:{}), (b:{}) where ID(a) = {} AND ID(b) = {} MERGE (a)-[r:{}]->(b) \
            ...
            parent_label, child_label, parent_id, child_id, relation
        )
        return parent_child_syntax

    @staticmethod
    def _create_execution_artifacts_link_syntax(
        parent_label: str,
        child_label: str,
        parent_id: int,
        child_id: int,
    ...
    @staticmethod
    def _create_execution_link_syntax(
        parent_label: str,
        child_label: str,
        parent_uri: str,
        parent_id: str,
    ...
    @staticmethod
    def _create_parent_child_artifacts_syntax(
        parent_label: str,
        child_label: str,
        parent_uri: str,
        child_uri: str,
        ...
        """
        parent_child_syntax_1 = (
            "MATCH (a:{}), (b:{}) WHERE a.uri = '{}' AND b.uri = '{}' ".format(
