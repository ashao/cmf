...
def dvc_get_url(folder: str, retry: bool = False, repo: str = "") -> str:
    ...
    try:
        ...
        if not retry:
            ...
            print(
                f"dvc.exceptions.PathMissingError Caught  Unexpected {err}, {type(err)}"
            )
...
def git_checkout_new_branch(branch_name: str):
    ...
    try:
        ...
        print(
            f"Checking out new branch for the execution failed, continuing in the default branch."
        )
    ...
    try:
        ...
        process = subprocess.Popen(
            [
                "git",
                "commit",
                "-m " + "commiting " + str(file_path) + "-" + str(execution_id),
            ],
            stdout=subprocess.PIPE,
            universal_newlines=True,
        )
        ...
        commit = output.strip()
        process = subprocess.Popen(
            ["git", "log", file_path], stdout=subprocess.PIPE, universal_newlines=True
        )
        # To-Do : Parse the output and report if error
        output, errs = process.communicate(timeout=60)
...
def git_commit(execution_id: str) -> str:
    ...
    try:
        # To-Do : Parse the output and report if error
        process = subprocess.Popen(
            ["git", "commit", "-m " + "commiting " + str(execution_id)],
            stdout=subprocess.PIPE,
            universal_newlines=True,
        )
        ...
        process = subprocess.Popen(
            ["git", "log"], stdout=subprocess.PIPE, universal_newlines=True
        )
        # To-Do : Parse the output and report if error
        output, errs = process.communicate(timeout=60)
        commit = output.splitlines()[0].strip()
...
def commit_output(folder: str, execution_id: str) -> str:
    ...
    try:
        if os.path.exists(os.getcwd() + "/" + folder):
            sub_dir_file = True
            process = subprocess.Popen(
                ["dvc", "add", folder], stdout=subprocess.PIPE, universal_newlines=True
            )
        else:
            sub_dir_file = False
            process = subprocess.Popen(
                ["dvc", "import-url", "--to-remote", folder],
                stdout=subprocess.PIPE,
                universal_newlines=True,
            )
        ...
        if process.returncode != 0:
            raise Exception(
                f"DVC add/import-url failed, Check if DVC is tracking parent directory: {errs}"
            )
...
def git_quiet_init() -> str:
    ...
    try:
        process = subprocess.Popen(
            ["git", "init", "-q"], stdout=subprocess.PIPE, universal_newlines=True
        )
        output, errs = process.communicate(timeout=60)
        commit = output.strip()
...
def git_add_remote(git_url) -> str:
    ...
    try:
        process = subprocess.Popen(
            ["git", "remote", "add", "cmf_origin", f"{git_url}"],
            stdout=subprocess.PIPE,
            universal_newlines=True,
        )
        output, errs = process.communicate(timeout=60)
...
def dvc_quiet_init() -> str:
    ...
    try:
        process = subprocess.Popen(
            ["dvc", "init", "-q"], stdout=subprocess.PIPE, universal_newlines=True
        )
        output, errs = process.communicate(timeout=60)
        commit = output.strip()
...
def dvc_add_remote_repo(repo_type, repo_path) -> str:
    ...
    try:
        process = subprocess.Popen(
            ["dvc", "remote", "add", "-d", "-f", f"{repo_type}", f"{repo_path}"],
            stdout=subprocess.PIPE,
            universal_newlines=True,
        )
        output, errs = process.communicate(timeout=60)
...
def dvc_push(file_list: Optional[List[str]] = None) -> str:
    ...
    if file_list is None:
        try:
            process = subprocess.Popen(
                ["dvc", "push"], stdout=subprocess.PIPE, universal_newlines=True
            )
            output, errs = process.communicate()
            commit = output.strip()
        ...
        try:
            process = subprocess.Popen(
                file_list, stdout=subprocess.PIPE, universal_newlines=True
            )
            output, errs = process.communicate()
            commit = output.strip()
