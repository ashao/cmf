...
class CmfRayLogger(Callback):
    ...
    def on_trial_start(self, iteration, trials, trial, **info):
        ...
        self.cmf_obj[trial_id] = cmf.Cmf(
            filepath=self.file_path, pipeline_name=self.pipeline_name
        )
        _ = self.cmf_obj[trial_id].create_context(pipeline_stage=self.pipeline_stage)
        execution_id = self.cmf_obj[trial_id].create_execution(
            execution_type=f"Trial_{trial_id}",
        ...
        if self.data_dir:
            _ = self.cmf_obj[trial_id].log_dataset(
                url=str(self.data_dir), event="input"
            )
    ...
    def on_trial_result(self, iteration, trials, trial, result, **info):
        ...
        _ = self.cmf_obj[trial_id].log_metric(
            metrics_name=f"Trial_{trial_id}_metrics",
        ...
        model_path = curr_res.get(
            "model_path", None
        )  # Track the model path if available
        ...
        if metric_value is not None:
            if self.best_metric_values[trial_id] is None:
                ...
                if (
                    self.order == "max"
                    and metric_value > self.best_metric_values[trial_id]
                ) or (
                    self.order == "min"
                    and metric_value < self.best_metric_values[trial_id]
                ):
    ...
    def on_trial_complete(self, iteration, trials, trial, **info):
        ...
        if best_metric_value is not None:
            ...
            heapq.heappush(
                self.heap,
                (
                    self.heap_comparator * best_metric_value,
                    trial_id,
                    best_metric_value,
                    best_model_path,
                    execution_id,
            ...
            _ = self.cmf_obj[trial_id].log_execution_metrics(
                metrics_name=f"Best_{self.metric}_Trial_{trial_id}",
                custom_properties={
                    f"Best_{self.metric}": best_metric_value,
        ...
        print(
            f"Trial {trial_id} completed, Commiting to CMF: with name Trial_{trial_id}_metrics"
        )
        print()
        ...
        if best_model_path:
            _ = self.cmf_obj[trial_id].log_model(
                path=best_model_path, event="input", model_name=f"{trial_id}_model"
            )
    ...
    def on_trial_error(self, iteration, trials, trial, **info):
        ...
        if self.cmf_run[trial_id]:
            _ = self.cmf_obj[trial_id].commit_metrics(f"Trial_{trial_id}_metrics")
            _ = self.cmf_obj[trial_id].log_execution_metrics(
                metrics_name=f"Trial_{trial_id}_Result",
    ...
    def on_experiment_end(self, trials, **info):
        ...
        """
        print(
            f"Marking top {self.top_n} trials with 'in_top_n = True' at experiment end."
        )

        # Log the top 'n' trials from the heap
        while self.heap:
            _, top_trial_id, top_metric_value, top_model_path, top_execution_id = (
            ...
            _ = self.cmf_obj[top_trial_id].update_execution(
                execution_id=top_execution_id, custom_properties={"in_top_n": True}
            )

            print(
                f"Execution {top_execution_id} for Trial {top_trial_id} updated with 'in_top_n = True'."
            )
