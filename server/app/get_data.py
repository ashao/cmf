...
async def get_model_data(mlmdfilepath, modelId):
    ...
    model_exe_df.drop(
        columns=["Python_Env", "Git_Start_Commit", "Git_End_Commit"], inplace=True
    )
...
def get_all_exe_ids(
    mlmdfilepath: str, pipeline_name: str = None
    ...
    executions = (
        pd.DataFrame()
    )  # df is emptied to store execution ids for next pipeline.
    if pipeline_name:
        executions = query.get_all_executions_in_pipeline(pipeline_name)
        if not executions.empty:
            execution_ids[pipeline_name] = executions[
                ["id", "Context_Type", "Execution_uuid", "Context_ID"]
            ]
        else:
...
def get_all_artifact_ids(
    mlmdfilepath: str, execution_ids, pipeline_name: str = None
) -> t.Dict[str, t.Dict[str, pd.DataFrame]]:
        ...
        if not execution_ids.get(pipeline_name).empty:
            ...
            if artifacts.empty:
                artifact_ids[pipeline_name] = (
                    pd.DataFrame()
                )  # { pipeline_name: {empty df} }
            else:
                artifact_ids[pipeline_name] = {}
                for art_type in artifacts["type"]:
                    filtered_values = artifacts.loc[
                        artifacts["type"] == art_type, ["id", "name"]
                    ]
...
def get_artifacts(mlmdfilepath, pipeline_name, art_type, artifact_ids):
    ...
    if query.get_pipeline_id(pipeline_name) != -1:
        ...
        for name in art_names:
            ...
            execution_type_name = (
                exe_type_name["execution_type_name"].drop_duplicates().tolist()
            )
            execution_type_name = [
                str(element).split('"')[1] for element in execution_type_name
            ]
    ...
    if os.path.exists(server_store_path):
        ...
        for i in mlmd_data["Pipeline"][0][
            ...
            for j in i["executions"]:
                if (
                    j["name"] != ""
                ):  # If executions have name , they are reusable executions
        ...
        if executions_server != []:
            list_executions_exists = list(
                set(executions_client).intersection(set(executions_server))
            )
...
def get_mlmd_from_server(server_store_path: str, pipeline_name: str, exec_id: str):
    ...
    df = pd.DataFrame()
    if (
        query.get_pipeline_id(pipeline_name) != -1
...
def get_lineage_data(
    server_store_path, pipeline_name, type, dict_of_art_ids, dict_of_exe_ids
):
    if type == "Artifacts":
        lineage_data = query_artifact_lineage_d3force(
            server_store_path, pipeline_name, dict_of_art_ids
        )
        """
        ...
        lineage_data = query_list_of_executions(
        ...
        lineage_data = query_visualization_ArtifactExecution(
            server_store_path, pipeline_name
        )
    return lineage_data
