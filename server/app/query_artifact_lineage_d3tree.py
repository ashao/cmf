...
def query_artifact_lineage_d3tree(
    mlmd_path: str, pipeline_name: str, dict_of_art_ids: dict
    ...
    for type_, df in dict_of_art_ids[pipeline_name].items():
        for index, row in df.iterrows():
            ...
            one_hop_parent_artifacts = query.get_one_hop_parent_artifacts_with_id(
                artifact_id
            )  # get immediate artifacts
            child_parent_artifact_id[artifact_id] = (
                []
            )  # assign empty dict for artifact with no parent artifact
            if not one_hop_parent_artifacts.empty:  # if artifact have parent artifacts
                child_parent_artifact_id[artifact_id] = list(
                    one_hop_parent_artifacts["id"]
                )
    ...
    for id_val in topo_sorted_nodes:  # topo_sorted_nodes = ['1','2','3','4']
        if (
            ...
            parent_dict[parents].append(
                {
                    "id": artifact_name_id_dict[id_val],
                    "parents": [
                        artifact_name_id_dict[parent] for parent in input_data[id_val]
                    ],
                }
            )
