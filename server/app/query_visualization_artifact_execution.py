...
async def query_visualization_artifact_execution(
    mlmd_path: str, pipeline_name: str, dict_art_id: dict, dict_exe_id: dict
) -> list:
    arti_exe_dict = (
        {}
    )  # Used to map artifact and execution ids with artifact and execution names
    dict_output = (
        {}
    )  # Used to establish parent-child relationship between artifacts and executions
    ...
    for i, df_row in df.iterrows():
        arti_exe_dict["e_" + str(df_row["id"])] = (
            "execution_name_"
            + df_row["Context_Type"]
            + ":"
            + df_row["Execution_uuid"][:4]
        )
    ...
    for type_, df in dict_art_id[pipeline_name].items():
        for df_index, df_row in df.iterrows():
            ...
            arti_exe_dict["a_" + str(df_row["id"])] = (
                "artifact_name_" + modify_artifact_name(df_row["name"], type_)
            )
                ...
                for data_index, data_row in data.iterrows():
                    ...
                    if data_row["Type"] == "INPUT":
                        ...
                        if "e_" + str(data_row["execution_id"]) in dict_output.keys():
                            dict_output["e_" + str(data_row["execution_id"])].append(
                                "a_" + str(df_row["id"])
                            )
                        else:
                            dict_output["e_" + str(data_row["execution_id"])] = [
                                "a_" + str(df_row["id"])
                            ]
                    else:
                        if "a_" + str(df_row["id"]) in dict_output.keys():
                            dict_output["a_" + str(df_row["id"])].append(
                                "e_" + str(data_row["execution_id"])
                            )
                        else:
...
def topological_sort(input_data: dict, arti_exe_dict: dict) -> list:
    ...
    for id_val in topo_sorted_nodes:  # topo_sorted_nodes = ['1','2','3','4']
        if (
            id_val in input_data
        ):  # input_data = {"child_id":[parents_id]}, for example {"4":['3','7','9']}
            ...
            parent_dict[parents].append(
                {
                    "id": arti_exe_dict[id_val],
                    "parents": [arti_exe_dict[parent] for parent in input_data[id_val]],
                }
            )
...
def modify_artifact_name(artifact_name: str, type: str) -> str:
    ...
    try:
        ...
        if (
            type == "Metrics"
        ):  # Example metrics:4ebdc980-1e7c-11ef-b54c-25834a9c665c:388 -> metrics:4ebd:388
            ...
            name = (
                dataslice_part1.rsplit(":", -1)[0]
                + ":"
                + dataslice_part1.rsplit(":", -1)[-1][:4]
            )
            ...
            name = (
                step_new.rsplit(":", -3)[0]
                + ":"
                + step_metrics_part2[-3][:4]
                + ":"
                + step_metrics_part2[-2]
                + ":"
                + step_metrics_part2[-1][:4]
            )
        else:
            name = artifact_name
