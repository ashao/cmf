...
def query_execution_lineage_d3force(
    mlmd_path, pipeline_name, dict_of_exe_ids, uuid_server
) -> Dict:
    """
    ...
    result = df[
        df["Execution_uuid"].str[:4] == uuid_server
    ...
    link_src_trgt_list = []
    host_id = None
    for id, context_type, uuid in zip(
        dict_of_exe_ids[pipeline_name]["id"],
        dict_of_exe_ids[pipeline_name]["Context_Type"],
        dict_of_exe_ids[pipeline_name]["Execution_uuid"],
    ):
        truncated_uuid = uuid.split("_")[0][
            :4
        ]  # first 4 characters of uuid example fb0e
        # comparing exec_type fetched from server with the one in dict_of_exe_ids and getting host_id of exec_type
        if (context_type + "_" + truncated_uuid) == (
            exec_type.to_list()[0] + "_" + uuid_server
        ):
            host_id = id
            node_id_name_list.append(
                {
                    "id": host_id,
                    "name": (
                        context_type.split("/")[-1] + "_" + uuid.split("-")[0][:4]
                    ),
                    "color": "#16B8E9",
                }
            )
    ...
    for i in exec_new[0]:  # appending id, name, exec_uuid to node_id_name_list
        ...
        node_id_name_list.append(
            {
                "id": i[0],
                "name": (name.split("/")[-1] + "_" + exec_uuid.split("-")[0][:4]),
                "color": "#FA6318",
            }
        )
    ...
    if not node_id_name_list and not link_src_trgt_list:
        return data
    node_id_name_list_unique = [
        dict(t) for t in {tuple(d.items()) for d in node_id_name_list}
    ]
    new_list = pd.DataFrame(link_src_trgt_list[0]).drop_duplicates().to_dict("records")
    data = {"nodes": node_id_name_list_unique, "links": new_list}
    return data
