...
def query_artifact_lineage_d3force(
    mlmd_path: str,
    pipeline_name: str,
    dict_of_art_ids: t.Dict[str, t.Dict[str, pd.DataFrame]],
) -> dict:
    art_name_id = {}
    artifact_name_list = []
    ...
    for type_, df in dict_of_art_ids[pipeline_name].items():
        for index, row in df.iterrows():
            ...
            result = (
                "#48D1CC"
                if type_ == "Dataset"
                else "#2E8B57" if type_ == "Model" else "#FF8C00"
            )
            node_id_name_list.append(
                {
                    "name": truncate_artifact_name(row["name"]),
                    "id": row["id"],
                    "color": result,
                }
            )
    ...
    for art_name in artifact_name_list:
        immediate_child_artifacts = query.get_one_hop_child_artifacts(
            art_name, pipeline_id
        )
    ...
    new_list = pd.DataFrame(link_list).drop_duplicates().to_dict("records")
    data = {"nodes": node_id_name_list, "links": new_list}
    return data
