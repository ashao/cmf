...
def query_execution_lineage_d3tree(
    mlmd_path: str, pipeline_name: str, dict_of_exe_id, uuid
):
    ...
    result = df[
        df["Execution_uuid"].str[:4] == uuid
    ]  # result = df[id: "1","Execution_type_name", "Execution_uuid"]
    execution_id = result["id"].tolist()
    parents_set = set()
    queue = UniqueQueue()
    df = pd.DataFrame()

    parents = query.get_one_hop_parent_executions_ids(
        execution_id, pipeline_id
    )  # list of parent execution ids
    dict_parents = {}
    if parents == None:
        parents = []
    dict_parents[execution_id[0]] = list(
        set(parents)
    )  # [2] = [1,2,3,4] list of parent id
    parents_set.add(
        execution_id[0]
    )  # created so that we can directly find execuions using execution ids
    ...
    df = query.get_executions_with_execution_ids(
        list(parents_set)
    ...
    result_dict = df.set_index("id")[
        "name_uuid"
    ].to_dict()  # {"id" : "name_uuid"} for example {"2":"Prepare_d09fdb26-0e9d-11ef-944f-4bf54f5aca7f"}

    data_organized = topological_sort(
        dict_parents, result_dict
    )  # it will use topological sort to create data from parents to child pattern
    """
    ...
    for id_val in topo_sorted_nodes:  # topo_sorted_nodes = ['1','2','3','4']
        if (
            ...
            parent_dict[parents].append(
                {
                    "id": modify_exec_name(execution_id_dict[id_val]),
                    "parents": [
                        modify_exec_name(execution_id_dict[parent])
                        for parent in input_data[id_val]
                    ],
                }
            )
