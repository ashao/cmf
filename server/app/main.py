...
async def lifespan(app: FastAPI):
    ...
    if os.path.exists(server_store_path):
        ...
        dict_of_art_ids = await async_api(
            get_all_artifact_ids, server_store_path, dict_of_exe_ids
        )
    yield
...
origins = ["http://localhost:3000", "localhost:3000", url]
...
async def mlmd_push(info: Request):
    ...
    if (
        pipeline_name not in pipeline_locks
    ):  # create lock object for pipeline if it doesn't exists in lock
        pipeline_locks[pipeline_name] = asyncio.Lock()
    pipeline_lock = pipeline_locks[pipeline_name]
    lock_counts[
        pipeline_name
    ] += 1  # increment lock count by 1 if pipeline going to enter inside lock section
    async with pipeline_lock:
        try:
            status = await async_api(
                create_unique_executions, server_store_path, req_info
            )
            ...
            lock_counts[
                pipeline_name
            ] -= 1  # Decrement the reference count after lock released
            if (
                lock_counts[pipeline_name] == 0
            ):  # if lock_counts of pipeline is zero means lock is release from it
                del pipeline_locks[
                    pipeline_name
                ]  # Remove the lock if it's no longer needed
...
async def mlmd_pull(info: Request, pipeline_name: str):
    ...
    if os.path.exists(server_store_path):
        ...
        json_payload = await async_api(
            get_mlmd_from_server, server_store_path, pipeline_name, req_info["exec_id"]
        )
    else:
        raise HTTPException(
            status_code=413, detail=f"mlmd file not available on cmf-server."
        )
    if json_payload == None:
        raise HTTPException(
            status_code=406, detail=f"Pipeline {pipeline_name} not found."
        )
    return json_payload
...
async def executions(
    ...
    if os.path.exists(server_store_path) and pipeline_name in dict_of_exe_ids:
        ...
        if filter_by and filter_value:
            exe_ids_initial = exe_ids_initial[
                exe_ids_initial[filter_by].str.contains(filter_value, case=False)
            ]
        # Apply sorting if provided
        exe_ids_sorted = exe_ids_initial.sort_values(
            by=sort_field, ascending=(sort_order == "asc")
        )
        ...
        return {"total_items": total_items, "items": executions_parsed}
    else:
        return
...
async def artifacts(
    ...
    if os.path.exists(server_store_path):
        ...
        if art_type in art_ids_dict:
            ...
            return {"total_items": 0, "items": None}
        # Apply filtering if provided
        if filter_by and filter_value:
            art_ids_initial = art_ids_initial[
                art_ids_initial[filter_by].str.contains(filter_value, case=False)
            ]
        # Apply sorting if provided
        art_ids_sorted = art_ids_initial.sort_values(
            by=sort_field, ascending=(sort_order == "asc")
        )
        ...
        artifact_df = await async_api(
            get_artifacts, server_store_path, pipeline_name, art_type, artifact_id_list
        )
        ...
        if artifact_df == None or artifact_df == {}:
            data_paginated = None
            total_items = 0
        return {"total_items": total_items, "items": data_paginated}
    else:
...
async def artifact_lineage(
    request: Request, pipeline_name: str
...
async def upload_file(
    ...
    try:
        file_path = os.path.join(
            "/cmf-server/data/tensorboard-logs", pipeline_name, file.filename
        )
...
async def model_card(
    request: Request, modelId: int, response_model=List[Dict[str, Any]]
):
    json_payload_1 = ""
    json_payload_2 = ""
    json_payload_3 = ""
    ...
    # checks if mlmd file exists on server
    if os.path.exists(server_store_path):
        model_data_df, model_exe_df, model_input_art_df, model_output_art_df = (
            await get_model_data(server_store_path, modelId)
        )
...
async def artifact_execution_lineage(request: Request, pipeline_name: str):
    ...
    if os.path.exists(server_store_path):
        query = cmfquery.CmfQuery(server_store_path)
        if pipeline_name in query.get_pipeline_names():
            response = await query_visualization_artifact_execution(
                server_store_path, pipeline_name, dict_of_art_ids, dict_of_exe_ids
            )
    return response
...
async def update_global_art_dict(pipeline_name):
    global dict_of_art_ids
    output_dict = await async_api(
        get_all_artifact_ids, server_store_path, dict_of_exe_ids, pipeline_name
    )
