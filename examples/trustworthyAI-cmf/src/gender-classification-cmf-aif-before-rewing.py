...
# The first step is to to download the `Aligned&cropped` images at the location
# mentioned above.
#
...
#     race: indicates the race of the person and can be from 0 to 4, denoting White,
...
#
# For this tutorial we will restict the images to contain `White` and `Others` races. We
# need to specify the unprivileged and priviledged groups to obtain various metrics from
# the aif360 toolbox. We set `White` as the priviledged group and `Others` as the
# unpriviledged group for computing the results with gender as the outcome variable that
# needs to be predicted. We set prediction as `female (1)` as the unfavorable label and
# `male (0)` as favorable label for the purpose of computing metrics, it does not have
# any special meaning in the context of gender prediction.
...
# ### Update the `image_dir` with the downloaded and extracted images location and
# ### specify the desired image size.
# The images are loaded and resized using opencv library. The following code creates
# three key numpy arrays each containing the raw images, the race attributes and the
# gender labels.
...
#
# Let us rescale the pixels to lie between $-1$ and $1$ and split the complete dataset
# into train and test sets.  We use $70$-$30$ percentage for train and test,
# respectively.
...
_ = metawriter.log_dataset(
    image_dir, "input", custom_properties={"number_of_images": N, "img_size": img_size}
)
_ = metawriter.log_dataset(
    "p_train.pt",
    "output",
    custom_properties={"number_of_Train_images": train_size, "img_size": img_size},
)
_ = metawriter.log_dataset(
    "p_test.pt",
    "output",
    custom_properties={
        "number_of_Train_images": (N - train_size),
        "img_size": img_size,
    },
)
...
# Next, we will create the pytorch train and test data loaders after transposing and
# converting the images and labels. The batch size is set to $1024$.
...
# Next,  we will train the model summarized above. `num_epochs` specifies the number of
# epochs used for  training. The learning rate is set to $0.001$. We will use the `Adam`
# optimizer to minimze the standard cross-entropy loss for classification tasks.
#
#
...
# Let's get the predictions of this trained model on the test and use them to compute
# various fariness metrics available in the aif360 toolbox.
...
# The wrapper function defined below can be used to convert the numpy arrays and the
# related meta data into a aif360 dataset. This will ease the process of computing
# metrics and comparing two datasets. The wrapper consumes the outcome array, the
# protected attribute array, information about unprivileged_groups and
# privileged_groups; and the favorable and unfavorable label to produce an instance of
# aif360's `BinaryLabelDataset`.
...
torch.save(
    original_traning_dataset, "original_traning_dataset.pt"
)  # save to log into cmf
...
# based on two BinaryLabelDatasets. The first dataset is the original one and the second
# is the output of the classification transformer (or similar). Later on we will use
# `BinaryLabelDatasetMetric` which computes based on a single `BinaryLabelDataset`.
...
result = dict(
    Classification_accuracy=classified_metric_nodebiasing_test.accuracy(),
