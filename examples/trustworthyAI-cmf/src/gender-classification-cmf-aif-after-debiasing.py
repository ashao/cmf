...
# by skin tone. In this tutorial, we will demonstrate the use of the aif360
# toolbox to study the differential performance of a custom classifier. We use a
# bias mitigating algorithm available in aif360 with the aim of improving a
# classfication model in terms of the fairness metrics. We will work with the
# UTK dataset for this tutorial. This can be downloaded from here:
...
metawriter_tf = cmf.Cmf(filename="mlmd", pipeline_name=pipeline_name, graph=graph)


# # Step 1: Load and Process Images The first step is to to download the
# `Aligned&cropped` images at the location mentioned above.
#
...
#
#     age: indicates the age of the person in the picture and can range from 0
#          to 116.
#     gender: indicates the gender of the person and is either 0 (male) or 1
#             (female).
#     race: indicates the race of the person and can be from 0 to 4, denoting
#           White, Black, Asian, Indian, and Others (like Hispanic, Latino,
#           Middle Eastern).
#     date&time: indicates the date and time an image was collected in the UTK
#                dataset.
#
# For this tutorial we will restict the images to contain `White` and `Others`
# races. We need to specify the unprivileged and priviledged groups to obtain
# various metrics from the aif360 toolbox. We set `White` as the priviledged
# group and `Others` as the unpriviledged group for computing the results with
# gender as the outcome variable that needs to be predicted. We set prediction
# as `female (1)` as the unfavorable label and `male (0)` as favorable label for
# the purpose of computing metrics, it does not have any special meaning in the
# context of gender prediction.
...
# and specify the desired image size.  The images are loaded and resized using
...
if not os.path.exists(image_dir):
    ...
    get_ipython().system(
        'curl -c ./cookie -Ls "https://drive.google.com/uc?export=download&id=0BxYys69jI14kYVM3aVhKS1VhRUk" > /dev/null'
    )
    get_ipython().system(
        "curl -Lb ./cookie \"https://drive.google.com/uc?export=download&confirm=`awk '/download/ {print $NF}' ./cookie`&id=0BxYys69jI14kYVM3aVhKS1VhRUk\" -o UTKFace.tar.gz"
    )
...
# dataset into train and test sets.  We use $70$-$30$ percentage for train and
# test, respectively.
...
train = torch.utils.data.TensorDataset(
    Variable(torch.FloatTensor(X_train.astype("float32"))),
    Variable(torch.LongTensor(y_train.astype("float32"))),
)
# train_loader = torch.utils.data.DataLoader(train, batch_size=batch_size, shuffle=True)
test = torch.utils.data.TensorDataset(
    Variable(torch.FloatTensor(X_test.astype("float32"))),
...
original_traning_dataset = dataset_wrapper(
    outcome=y_train,
    protected=p_train,
...
# # Step 3: Apply the Reweighing algorithm to tranform the dataset Reweighing is
# a preprocessing technique that weights the examples in each (group, label)
# combination differently to ensure fairness before classification [1]. This is
# one of the very few pre-processing methods we are aware of that could
# tractably be applied to multimedia data (since it does not work with the
# features).
#
# References: [1] F. Kamiran and T. Calders,"Data Preprocessing Techniques for
...
metric_orig_train = BinaryLabelDatasetMetric(
    original_traning_dataset,
    unprivileged_groups=unprivileged_groups,
    privileged_groups=privileged_groups,
)
metric_tranf_train = BinaryLabelDatasetMetric(
    transf_traning_dataset,
    unprivileged_groups=unprivileged_groups,
    privileged_groups=privileged_groups,
)
...
# np.save('transf_traning_dataset', transf_traning_dataset)
...
print(
    "Difference in mean outcomes between privileged and unprivileged groups = %f"
    % metric_orig_train.mean_difference()
)
display(Markdown("#### Transformed training dataset"))
print(
    "Difference in mean outcomes between privileged and unprivileged groups = %f"
    % metric_tranf_train.mean_difference()
)
...
# # Step 4: Learn a New Classfier using the Instance Weights We can see that the
# reweighing was able to reduce the difference in mean outcomes between
# privileged and unprivileged groups. This was done by learning appropriate
# weights for each training instance. In the current step, we will use these
# learned instance weights to train a network. We  will create a custom pytorch
# loss called `InstanceWeighetedCrossEntropyLoss` that uses the instances
# weights to produce the loss value for a batch of data samples.

tranf_train = torch.utils.data.TensorDataset(
    Variable(torch.FloatTensor(X_train.astype("float32"))),
    Variable(torch.LongTensor(transf_traning_dataset.labels.astype("float32"))),
    Variable(
        torch.FloatTensor(transf_traning_dataset.instance_weights.astype("float32"))
    ),
)
tranf_train_loader = torch.utils.data.DataLoader(
    tranf_train, batch_size=1024, shuffle=True
)
        ...
        if (idx + 1) % print_freq == 0:
            print(
                "Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}".format(
                    epoch + 1, num_epochs, idx + 1, num_batches, loss.item()
                )
            )
...
# Let us repeat the same steps as before to convert the predictions into aif360 dataset
# and obtain various metrics.
transf_predictions_test_dataset = dataset_wrapper(
    outcome=y_pred_transf,
    protected=p_test,
    unprivileged_groups=unprivileged_groups,
    privileged_groups=privileged_groups,
    favorable_label=favorable_label,
...
# cmf logging of evaluation  ...
_ = metawriter_tf.create_context(pipeline_stage="Fairness_tranf_AD")
_ = metawriter_tf.create_execution(
    execution_type="Fairness_tranf_Evaluate-AD-execution"
)
_ = metawriter_tf.log_dataset("transf_predictions_test_dataset.pt", "input")
_ = metawriter_tf.log_dataset("original_test_dataset.pt", "input")

# Since the reweighing algorithm only reduces the difference in mean outcomes, it
# doesn't guarantee that every classification metric will be reduced for the debiased
# model. So your mileage may vary based on how the dataset was split and how the model
# was trained.
## cmf query to get metrics logged before debiasing transform
## cmf query for geting artifact and metrics
...
print(
    ...
    % classified_metric_nodebiasing_test["Classification_accuracy"]
)
print(
    "Test set: Balanced classification accuracy = %f"
    % classified_metric_nodebiasing_test["Balanced_classification_accuracy"]
)
print(
    "Test set: Statistical parity difference = %f"
...
print(
    "Test set: False negative rate difference = %f"
...
result_tranf = dict(
    Classification_accuracy=classified_metric_debiasing_test.accuracy(),
...
_ = metawriter_tf.log_execution_metrics("transf_metrics_ad", result_tranf)

# Let us break down these numbers by age to understand how these bias differs across age
# groups. For demonstration, we dividee all the samples into these age groups: 0-10,
# 10-20, 20-40, 40-60 and 60-150. For this we will create aif360 datasets using the
# subset of samples that fall into each of the age groups. The plot below shows how the
# `Equal opportunity difference` metric varies across age groups before and after
# applying the bias mitigating reweighing algorithm.
...
# stage_evaluate = pipeline_name + '/' + 'Evaluate'
# executions = query.get_all_executions_in_stage(stage_evaluate)
...
for idx in range(len(age_range_intervals) - 1):
    ...
    true_dataset = dataset_wrapper(
        outcome=y_test[ids],
        protected=p_test[ids],
        unprivileged_groups=unprivileged_groups,
        privileged_groups=privileged_groups,
        favorable_label=favorable_label,
        unfavorable_label=unfavorable_label,
    )
    transf_pred_dataset = dataset_wrapper(
        outcome=y_pred_transf[ids],
        protected=p_test[ids],
        unprivileged_groups=unprivileged_groups,
        privileged_groups=privileged_groups,
        favorable_label=favorable_label,
    ...
    nodebiasing_perf.append(
        classified_metric_nodebiasing_test.equal_opportunity_difference()
    )
    debiasing_perf.append(
        classified_metric_debiasing_test.equal_opportunity_difference()
    )
...
p2 = ax.bar(ind + width, debiasing_perf, width, color="y")
ax.set_title("Equal opportunity difference by age group")
ax.set_xticks(ind + width / 2)
ax.set_xticklabels(
    [
        str(age_range_intervals[idx]) + "-" + str(age_range_intervals[idx + 1])
        for idx in range(N)
    ]
)
...
# plt.show()
plt.savefig("bias_before_after_adversialdebias_fig.jpg")
print("***** finished ******")
plt.close(fig)

# # Conclusions
# In this tutorial, we have examined fairness in the scenario of binary classification
# with face images. We discussed methods to process several attributes of images,
# outcome variables, and protected attributes and created aif360 ready dataset objects
# on which many bias mitigation algorithms can be easily applied and fairness metrics
# can be easliy computed. We used the reweighing algorithm with the aim of improving the
# algorithmic fairness of the learned classifiers. The empirical results show slight
# improvement in the case of debiased model over the vanilla model. When sliced by age
# group, the results appear to be mixed bag and thus has scope for further improvements
# by considering age groups while learning models.
