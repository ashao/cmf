...
class Data:
    ...
    def get_unlabeled_data_subset(self, subset_percentage):
        subset = int(subset_percentage * self.n_pool)
        tmp_idxs = np.arange(self.n_pool)[~self.labeled_idxs]
        np.random.shuffle(tmp_idxs)
        unlabeled_idxs = tmp_idxs[:subset]
        return unlabeled_idxs, self.handler(
            self.X_train[unlabeled_idxs], self.Y_train[unlabeled_idxs]
        )
...
def get_MNIST(handler):
    ...
    # return Data(raw_train.data[:10], raw_train.targets[:1000], raw_test.data[:1000], raw_test.targets[:1000], handler)
